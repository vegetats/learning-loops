...
function clientTypeAccount(typeOfClient, persons, accounts) {
    let filteredClients = [];

    switch(typeOfClient) {
        case 'Gold':
            filteredClients = accounts.filter(acc => acc.clientType === 'Gold')
            break;
        case 'Platinum':
            filteredClients = accounts.filter(acc => acc.clientType === 'Platinum')
            break;
        case 'Silver':
            filteredClients = accounts.filter(acc => acc.clientType === 'Silver')
            break;
        case 'Basic':
            filteredClients = accounts.filter(acc => acc.clientType === 'Basic')
            break;
...
clientTypeAccount('Gold', personalInfos, accountInfos); 